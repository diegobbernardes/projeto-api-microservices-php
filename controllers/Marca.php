	...
	class Marca{
		...
		*/
		...
		public function lista(){
			global $app;
			$sth = $this->PDO->prepare("SELECT * FROM dtweb289_componentes.marca;");
			$sth->execute();
		...
		public function editar($id){
			...
			$sth = $this->PDO->prepare("UPDATE dtweb289_componentes.marca SET ".implode(',', $sets)." WHERE id = :id");
			$sth ->bindValue(':id',$id);
		...
		public function nova(){
			...
			*/
			$sth = $this->PDO->prepare("INSERT INTO dtweb289_componentes.marca (".implode(',', $keys).") VALUES (:".implode(",:", $keys).")");
			foreach ($dados as $key => $value) {
				$sth ->bindValue(':'.$key,$value);
			}
			$sth->execute();
		...
		*/
		public function get($id){
			global $app;
			$sth = $this->PDO->prepare("SELECT * FROM dtweb289_componentes.marca where id = :id");
			$sth ->bindValue(':id',$id);
			$sth->execute();
		...
		public function excluir($id){
			global $app;
			$sth = $this->PDO->prepare("DELETE FROM dtweb289_componentes.marca where id = :id");
			$sth ->bindValue(':id',$id);
			$app->render('default.php',["data"=>['status'=>$sth->execute()==1]],200); 
		}
